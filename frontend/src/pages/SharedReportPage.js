  ...
  return (
    <div className="page-shell min-h-screen max-w-6xl" aria-busy="true" aria-live="polite" aria-label="Loading shared report">
      ...
      <div className="mx-auto max-w-md text-center">
        <p className="text-sm font-semibold text-aura-ink dark:text-slate-200">Opening shared scorecard…</p>
        <p className="mt-1.5 text-xs leading-relaxed text-aura-muted dark:text-slate-400">
          Verifying the link and loading the session the candidate shared.
        </p>
...
function QuestionCard({ question, index }) {
  ...
  const panelId = `shared-q-panel-${index}`;
  const headerId = `shared-q-header-${index}`;
