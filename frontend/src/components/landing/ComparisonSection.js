...
function YesNo({ value }) {
    ...
    return (
      <div className="flex justify-center py-1">
        {value ? (
          ...
          <span
            className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-400 ring-1 ring-slate-200"
            title="No"
            aria-label="No"
          >
            —
  ...
  return <p className="py-1 text-center text-sm font-medium text-slate-600">{value}</p>;
}

export default function ComparisonSection() {
  return (
    <section className="relative py-24 md:py-28">
      <div className="pointer-events-none absolute inset-x-0 top-1/2 h-[min(80%,480px)] -translate-y-1/2 bg-gradient-to-r from-violet-50/40 via-transparent to-orange-50/40 blur-3xl" aria-hidden />
            ...
            <table className="w-full min-w-[320px] border-collapse text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/95 text-left">
                  <th className="px-4 py-4 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 sm:px-6">Capability</th>
                  <th className="px-3 py-4 text-center font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-violet-700 sm:px-5">
                    InterviewAI
                  </th>
                  <th className="px-3 py-4 text-center font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 sm:px-5">
                    Typical banks
                  </th>
                </tr>
              </thead>
              <tbody>
                {COMPARISON_ROWS.map((row) => (
                  <tr key={row.label} className="border-b border-slate-100 bg-white last:border-0">
