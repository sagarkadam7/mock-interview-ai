  ...
  return (
    <div className="border-b border-slate-200/90 last:border-0">
      <button
        ...
        onClick={onToggle}
        className="flex w-full items-start justify-between gap-4 py-5 text-left transition-colors hover:text-violet-800"
      >
        <span className="text-[15px] font-semibold text-aura-ink">{item.q}</span>
        <span className={`mt-0.5 shrink-0 text-slate-400 transition-transform duration-200 ${open ? "rotate-180" : ""}`} aria-hidden>
          ▼
        </span>
      </button>
...
export default function FAQSection({ limit = null }) {
  ...
  return (
    <section id="faq" className="scroll-mt-24 border-t border-slate-200/80 bg-white py-24 md:py-28">
      <div className="mx-auto max-w-3xl px-6">
        <div className="mb-12 text-center">
          ...
          <p className="mx-auto mt-4 max-w-lg text-[15px] text-slate-600">Security, browsers, and how AI fits into real prep.</p>
        </div>
