...
export default function HowItWorksSection() {
  return (
    <section id="how-it-works" className="scroll-mt-24 border-y border-slate-200/80 bg-gradient-to-b from-white via-slate-50/50 to-white py-24 md:py-28">
          ...
          {HOW_STEPS.map((step, i) => (
            ...
            >
              <div className="glass-panel-lg relative h-full overflow-hidden rounded-2xl p-6 md:p-7">
                <span className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">{step.n}</span>
                <h3 className="mt-3 text-lg font-bold tracking-tight text-aura-ink">{step.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">{step.desc}</p>
                {i < HOW_STEPS.length - 1 && (
                  <div
                    className="pointer-events-none absolute -right-3 top-1/2 hidden h-px w-6 -translate-y-1/2 bg-gradient-to-r from-slate-200 to-transparent lg:block"
                    aria-hidden
                  />
                )}
              </div>
