...
export default function LandingHero({ user }) {
  ...
  const inlineDemoVideo = isInlineVideo(DEMO_VIDEO_URL);
  ...
  return (
    ...
    >
        ...
        >
          ...
          <motion.div variants={item} className="mt-9 flex w-full max-w-none flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-start sm:gap-6">
              {hasDemoVideo && inlineDemoVideo ? (
                <button type="button" className={demoWatchBtnClass} onClick={() => setVideoOpen(true)}>
                  <DemoPlayIcon />
                  Watch demo
                </button>
              ) : hasDemoVideo && isExternalPage(DEMO_VIDEO_URL) ? (
                <a
                  href={DEMO_VIDEO_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={demoWatchBtnClass}
                >
                  <DemoPlayIcon />
                  Watch demo
                </a>
              ) : (
                <a href="#how-it-works" className={demoWatchBtnClass}>
                  <DemoPlayIcon />
                  See how it works
                </a>
              )}
      ...
      <VideoModal
        ...
        src={DEMO_VIDEO_URL}
        title="InterviewAI — product demo"
      />
    </section>
  );
}
